...
# Load Rules
engine = RuleEngine("rules/uae_pint_ae_rules.json")
validator = InvoiceValidator(engine)
...
b1_payload = {
    ...
    "lines": [{
        ...
        "tax_rate": 0.05,
        "tax_amount": 250.0,
        "vat_line_amount_aed": 250.0,
        "line_amount_aed": 5000.0
    }],
    "tax_subtotals": [{
        "tax_category_code": "S",
        "tax_rate": 0.05,
...
try:
    payload = InvoicePayload(**b1_payload)
